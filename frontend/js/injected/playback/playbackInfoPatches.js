...
(function (window) {
    ...
    function enforceMaxBitrateUrl(url, maxBitrate, bitrateParamName) {
        ...
        var existingBitrate = parsePositiveInteger(getQueryParameterValue(url, bitrateParamName || DEFAULT_MAX_BITRATE_PARAM));
        ...
        return {
            url: setQueryParameterValue(url, bitrateParamName || DEFAULT_MAX_BITRATE_PARAM, targetBitrate),
    ...
    function patchPlaybackInfoBitrateObject(value, normalizedTarget, options, visited) {
        ...
        for (var i = 0; i < keys.length; i++) {
            ...
            if (currentBitrate < normalizedTarget) {
                ...
                changed = true;
                debugLog(options, 'Patched PlaybackInfo body bitrate (' + source + ', ' + key + '): ' + currentBitrate + ' -> ' + normalizedTarget);
            }
        }
        ...
        for (var j = 0; j < nestedKeys.length; j++) {
            if (patchPlaybackInfoBitrateObject(value[nestedKeys[j]], normalizedTarget, options, visited)) {
                changed = true;
            }
        }
